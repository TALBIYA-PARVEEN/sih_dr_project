...
async function initOfficialGoogleSignIn() {
    ...
    btnContainer.innerHTML = `
        <button type="button" onclick="promptGoogleClientIdSetup()" class="w-full py-2.5 px-4 bg-white hover:bg-slate-50 border border-slate-300 text-slate-700 font-semibold rounded-xl text-xs shadow-xs transition flex items-center justify-center space-x-2.5">
...
function promptGoogleClientIdSetup() {
    const input = window.prompt("To enable Official Google Sign-In, please paste your Google Cloud OAuth Client ID (e.g. 123456-abc.apps.googleusercontent.com):");
    if (input && input.trim() && input.includes(".apps.googleusercontent.com")) {
        localStorage.setItem("netra_google_client_id", input.trim());
        activeGoogleClientId = input.trim();
        showToast("Google Client ID saved! Initializing Google Sign-In...", "success");
        initOfficialGoogleSignIn();
    } else if (input !== null) {
        showToast("Please enter a valid Client ID ending with .apps.googleusercontent.com", "warning");
    }
}
